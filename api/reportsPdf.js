...
// Every stamp in these PDFs is rendered in the SERVER's local timezone - Node
// gets no TZ override here and the services run as LocalSystem, so it inherits
// the machine's zone. The on-screen report renders in the BROWSER's zone
// instead, so the two only agree while reader and server share a zone. A
// scheduled report is emailed and read away from that machine, so print the
// zone on the cover rather than leaving it implied.
const TZ_LABEL = (function () {
  try {
    const zone = Intl.DateTimeFormat().resolvedOptions().timeZone || '';
    const parts = new Intl.DateTimeFormat('en-GB', { timeZoneName: 'short' }).formatToParts(new Date());
    const abbr = (parts.find(function (x) { return x.type === 'timeZoneName'; }) || {}).value || '';
    if (zone && abbr) return zone + ' (' + abbr + ')';
    return zone || abbr || 'server local time';
  } catch (_e) { return 'server local time'; }
})();

// Window label for the granular detail reports (AP / device / service). Their
// range comes from datetime-local pickers, so the TIME the user picked is part
// of what they asked for. Printing only the date collapsed an 08:00-17:00
// window into "18/08/2026 to 18/08/2026" - indistinguishable from the whole
// day, while the on-screen report showed the times. Times are dropped only
// when the window is exactly whole-day aligned, where they carry nothing.
function fmtClock(d) {
...
function fmtRangeLabel(from, to) {
  ...
  const b = new Date(to);
  if (isNaN(a.getTime()) || isNaN(b.getTime())) return fmtDay(from) + ' to ' + fmtDay(to);
  // Midnight start, ending either at midnight or at the 23:59:59 a whole-day
  // picker produces. Anything else is a time the user deliberately chose.
  const startsAtMidnight = a.getHours() === 0 && a.getMinutes() === 0;
  const endsAtDayEdge = (b.getHours() === 0 && b.getMinutes() === 0)
    || (b.getHours() === 23 && b.getMinutes() === 59);
  if (startsAtMidnight && endsAtDayEdge) return fmtDay(a) + ' to ' + fmtDay(b);
  return fmtDay(a) + ' ' + fmtClock(a) + ' to ' + fmtDay(b) + ' ' + fmtClock(b);
}
...
async function gatherDeviceDetail(db, params) {
  ...
  const dateRange = fmtRangeLabel(range.from, range.to);
