...
const releaseNotes = {
  ...
  '1.107.1': [
    ...
    'Fixed: opening the alert editor on a sensor row squeezed the sensor\'s own name out of the row. The editor now opens on its own line underneath the sensor, and shows which sensor it is about.',
  ],
  '1.107.0': [
  ...
  '1.86.9': [
    ...
    'Documented two vendor-specific measurements that cannot be verified on this network because no such equipment is installed here: on Cisco controllers the "retry rate" figure actually carries channel utilization, which makes the high-retry alert mean something different on Cisco than on Aruba; and the MikroTik client-to-access-point matching relies on an assumption no MikroTik device has ever confirmed. Both are unchanged and flagged in the code, to be settled when such hardware is first connected.',
  ],
  '1.86.8': [
  ...
  '1.86.1': [
    ...
    'The database statement now casts its parameters explicitly. Verified against the live production database: the previous statement is rejected before it runs, the corrected one plans and executes.',
  ],
  '1.86.0': [
  ...
  '1.56.0': [
    ...
    'Reports have a proper report header/cover (title, brand, scope/range, "generated by … on …") for a more professional exported PDF, and all report templates now share one consistent style set',
    'Site detail page modernised to match the dashboard/device pages (KPI tiles, skeleton loaders, consistent status colours)',
    'Replaced emoji used as status/icons with crisp SVG icons and status dots across the dashboard, alerts, wireless, and map editor for a cleaner, more consistent look',
  ],
  '1.55.0': [
...
// Latest short commit hash on origin/main via GIT transport. Uses git instead
// of GitHub's public web APIs because those are per-IP rate-limited — from a
...
// (checked over git transport). ANY differing commit counts as an update
// available — package.json version is for display only. Never 500s the Settings
...
app.get('/api/system/update-status', wrap(async (_req, res) => {
  ...
  try {
    ...
    // (an extra git fetch) when a new commit actually exists; otherwise the
