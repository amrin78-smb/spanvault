...
- Deploy by running this app's own `installer/Update-SpanVault.ps1` over WinRM
  `Invoke-Command`, never by hand-editing the server. Pass
  `-InstallDir C:\Apps\SpanVault\app` — the install root is the `app` **subfolder**
  (`C:\Apps\SpanVault` itself holds only `app`/`logs`/`nssm`) — and OMIT `-ServerIp`
  on a routine update; it's for first install only and the script preserves both
  `.env.local` files. Run it backgrounded: an npm install + Next build outlasts a
  foreground tool timeout, and timing out mid-deploy tears down the WinRM session.
  The updater snapshots the commit + `node_modules` + `.next` and
...
- Always verify **after** the deploy, and verify *independently* rather than
  ...
  check is what caught this app's own 1.88.1 site-header bug, where a stretched
  link swallowed 68% of the header's clicks. Every static check passed it, and it
  was invisible in a screenshot.
