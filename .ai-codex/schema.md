...
- `alert_rules`  id(PK,SERIAL) | device_id — FK->monitored_devices CASCADE (nullable) | metric,operator,threshold(nullable),severity,enabled,created_at | scope('global'/'site'/'device'/'service'),site_id,site_name,notify_recovery,description | service_check_id — FK->service_checks CASCADE | sensor_key,sensor_label (nullable; set = a PER-SENSOR rule, and `metric` then holds that sensor's STORED metric_name — the per-index suffixed form like `node_test_ok_1596274411` — not one of the fixed device metrics. sensor_label is a display snapshot so the rule still reads correctly if the sensor goes away. Requires device_id; enforced in POST /api/alert-rules)
